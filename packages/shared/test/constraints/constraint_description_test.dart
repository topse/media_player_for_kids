...
void main() {
  late ConstraintDescriptionGenerator gen;

  setUpAll(() async {
    await initializeDateFormatting();
    final loc = await SharedL10n.delegate.load(const Locale('de'));
    gen = ConstraintDescriptionGenerator(loc);
  });

  group('ConstraintDescriptionGenerator (de)', () {
    group('PlayCountConstraint', () {
      test('maxCount == 1 perDay → "Einmal pro Tag"', () {
        ...
        expect(gen.describe(c), 'Einmal pro Tag');
      });
      ...
      test('maxCount == 3 perDay → "Maximal 3× pro Tag"', () {
        ...
        expect(gen.describe(c), 'Maximal 3× pro Tag');
      });
      ...
      test('maxCount == 1 rollingHours 24 → "Einmal je 24 Stunden"', () {
        ...
        expect(gen.describe(c), 'Einmal je 24 Stunden');
      });
      ...
      test('perWeek → "pro Woche"', () {
        ...
        expect(gen.describe(c), 'Maximal 2× pro Woche');
      });
      ...
      test('perMonth → "pro Monat"', () {
        ...
        expect(gen.describe(c), 'Maximal 5× pro Monat');
      });
      ...
      test('sinceDate → formatted date in label', () {
        ...
        expect(gen.describe(c), 'Einmal seit 1.12.2026');
      });
    });
    ...
    group('PlayDurationConstraint', () {
      test('30 min perWeek → "Max. 30 Min. pro Woche"', () {
        ...
        expect(gen.describe(c), 'Max. 30 Min. pro Woche');
      });
    });
    ...
    group('FolderItemCountConstraint', () {
      test('2 items perDay → "Max. 2 Einträge pro Tag"', () {
        ...
        expect(gen.describe(c), 'Max. 2 verschiedene Einträge pro Tag');
      });
    });
    ...
    group('TimeOfDayConstraint', () {
      test('"Nur 08:00–20:00 Uhr"', () {
        ...
        expect(gen.describe(c), 'Nur 08:00–20:00 Uhr');
      });
    });
    ...
    group('Logical operators', () {
      test('AND joins with " und "', () {
        ...
        expect(gen.describe(c), 'Nur Mo–Fr und Nur 08:00–20:00 Uhr');
      });
      ...
      test('OR joins with " oder "', () {
        ...
        expect(gen.describe(c), 'Nur Mo–Fr oder Nur am Wochenende');
      });
      ...
      test('NOT prefixes with "Nicht: "', () {
        ...
        expect(gen.describe(c), 'Nicht: Nur 08:00–20:00 Uhr');
      });
      ...
      test('nested AND inside NOT', () {
        ...
        expect(gen.describe(c), 'Nicht: Nur Mo–Fr');
      });
    });
  });

  group('ConstraintDescriptionGenerator (en)', () {
    late ConstraintDescriptionGenerator en;

    setUpAll(() async {
      final loc = await SharedL10n.delegate.load(const Locale('en'));
      en = ConstraintDescriptionGenerator(loc);
    });

    test('PlayCount maxCount==1 perDay → "Once per day"', () {
      const c = PlayCountConstraint(
        maxCount: 1,
        window: TimeWindow(type: TimeWindowType.perDay),
      );
      expect(en.describe(c), 'Once per day');
    });

    test('DayOfWeek weekdays → "Only Mon–Fri"', () {
      const c = DayOfWeekConstraint(allowedDays: [1, 2, 3, 4, 5]);
      expect(en.describe(c), 'Only Mon–Fri');
    });
    ...
    test('AND joins with " and "', () {
      ...
      expect(en.describe(c), 'Only Mon–Fri and Only 08:00–20:00');
    });
  });
}
